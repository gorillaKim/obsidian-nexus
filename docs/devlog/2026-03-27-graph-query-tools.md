...
## 트러블슈팅: nexus_find_path null 반환 — 미해결 위키링크

### 현상
`nexus_find_path(overview/project-overview.md → architecture/database-schema.md)` 호출 시
null 반환. `nexus_get_cluster`에서는 두 문서가 2-hop으로 연결된 것으로 나타남에도 불구하고
경로를 찾지 못함.

### 원인
`nexus_find_path`는 `wiki_links` 테이블의 `target_doc_id IS NOT NULL`인 링크,
즉 **resolve된 링크만** BFS로 탐색한다.
`overview/project-overview.md`의 위키링크들이 `[[04-데이터베이스-스키마]]` 형식의
표시 이름으로 작성되어 있어 파일 경로 resolve 실패 (`target_doc_id = NULL`).

`nexus_get_links` 결과:
```json
{ "target_path": "04-데이터베이스-스키마", "resolved": false }
```

### 해결 방법
위키링크를 파일 경로 기반 형식으로 작성해야 resolve 성공:

| ❌ resolve 실패 | ✅ resolve 성공 |
|----------------|----------------|
| `[[04-데이터베이스-스키마]]` | `[[architecture/database-schema]]` |
| `[[03-MCP-도구-레퍼런스]]` | `[[integrations/mcp-tools]]` |

`nexus_get_cluster`는 양방향 탐색 특성상 일부 케이스에서 비resolve 링크도 추적할 수
있지만, `nexus_find_path`는 순방향 BFS 특성상 resolve된 링크만 따른다.

### 사전 확인 방법
`nexus_get_links`로 `"resolved": false` 비율을 먼저 확인.
false가 많으면 `nexus_find_path` 결과가 불완전하거나 null이 될 수 있음.

## CLI graph 서브커맨드 추가

MCP 도구 3종과 동일한 기능을 CLI에서도 사용할 수 있도록 `obs-nexus graph` 서브커맨드를 추가했다.

### 구현

- `crates/cli/src/commands/graph.rs` 신규 생성
- `GraphCommands` enum: `Related`, `Path`, `Cluster`
- `main.rs`에 `Commands::Graph` 등록
- `commands/mod.rs`에 `pub mod graph` 추가

### 사용법

```bash
obs-nexus graph related <project> <path> [--k N]
obs-nexus graph path    <project> <from> <to>
obs-nexus graph cluster <project> <path> [--depth N]
```

`--format text` 옵션으로 human-readable 출력, 기본값은 JSON.
